...
update staff set staff.available='yes' where s_id = (select s_id from delivers natural join orders where o_id = (SELECT * FROM most_recent));

select s_id from staff natural join delivers natural join orders where o_id = (SELECT * FROM most_recent);

update orders set o_status = 'Delivered' where o_id = (select * from most_recent);
